...
class ConfigManager {
	...
	public function getValue($key) {
		if(!empty($this->currentValues[$key])) {
			return $this->currentValues[$key]['value'];
		}
			...
			foreach($this->currentValues as $key=>$value) {
				...
				if($value['type'] == "STRING") {
					...
					if(strtolower($value['value']) == "true") {
						...
						if($this->defaultValues[$key]['value']) {
							$inhalt .= '$conf[\'' . $key . '\'] = true;' . PHP_EOL;
						} else {
							$inhalt .= '$conf[\'' . $key . '\'] = false;' . PHP_EOL;
						}
	...
	private function initDefaultValues() {
		...
		$this->defaultValues['remote_authtokens']['type']        = "ARRAY";
		$this->defaultValues['remote_authtokens']['value']       = array();
		$this->defaultValues['remote_authtokens']['description'] = "Die Authtokens sind zur Kommunikation von externen Servern mit diesem notwendig.";
		
		$this->defaultValues['remote_authtoken_master']['type']        = "STRING";
		$this->defaultValues['remote_authtoken_master']['value']       = "";
		$this->defaultValues['remote_authtoken_master']['description'] = "Der Master Authtoken für die Kommunikation mit michaelsoftware.net .";
	}
